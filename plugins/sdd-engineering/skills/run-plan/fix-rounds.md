...
|---|---|---|---|---|
| 1 | major | introduced | `src/modules/blast/service.ts:88` | **in the brief** |
| 2 | major | pre-existing | `src/modules/pulls/routes.ts:41` | filter 1 — backlog |
...
| "I'll put the brief next to the plan so it gets committed" | § *Where the brief goes* — it moves the working-tree fingerprint and a push gate then refuses |
...
| "I'll just write the gate's category instead of the command" | § *The brief* — six wasted runs across two rounds, same flake, same entry |
