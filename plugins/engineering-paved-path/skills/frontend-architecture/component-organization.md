...
Split on a **symptom**, never on a number:
...
Three complementary ways to decide, from the React docs:
...
|---|---|
| server data | the query cache — it is not client state |
| bookmarkable or shareable (filters, pagination, search, tabs) | URL search params |
| used by one component | `useState` there |
| used by siblings | `useState` in their **closest** common ancestor — no higher |
| derivable from props or other state | **nowhere** — compute during render |
