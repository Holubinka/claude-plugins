...
**If you are uncertain, it is refuted.**
...
- `confidence: "uncertain"` → **set `refuted: true`.** That is the rule above, applied.
...
- **In:** one finding — the anchor with its line pasted, the invariant in the reviewer's own words, the input said to trigger it, and the diff base.
